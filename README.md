...
2. **Export supported active OpenCode and OhMy configuration:** Follow the literal checklist in [Create a pack](./CONFIG_PACK_RESTORE_GUIDE.md#create-a-pack). Always pass the exact source operating-system home with `--home`; never change the shell's `HOME` variable or guess a home. On Windows, never use `$home` either—PowerShell treats `$home` and `$HOME` as the same variable. Use `$sourceHome` exactly as documented. Use one complete command lane—installed command or repository checkout—for both `create` and `inspect`. The result is one sanitized ZIP migration template, not a working backup; it contains no credentials or endpoints and excludes raw launchers. Do not ask the route-selection question for this read-only export.
3. **Import or review a received archive:** Start with [Inspect before extracting](./CONFIG_PACK_RESTORE_GUIDE.md#inspect-before-extracting), then follow the approval-gated [Manual installation procedure for another agent](./CONFIG_PACK_RESTORE_GUIDE.md#manual-installation-procedure-for-another-agent). Use `$recipientHome` exactly as documented—never `$home` or `$HOME` in Windows PowerShell. Use one complete installed or checkout command lane, pass the exact recipient home to `extract`, and extract only to a new review directory. Inspection/extraction is not installation approval. Never overwrite live config or install placeholders, credentials, routes, plugins, or machine-specific values without exact owner approval and rollback backups. The route-selection exception applies only while the request is limited to `omo-model-pack create`, `omo-model-pack inspect`, or `omo-model-pack extract`; installation and route changes follow the normal rules.
