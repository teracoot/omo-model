...
2. **Export configuration—owner chooses one mode:** For a trusted friend who must receive an exact Windows environment, including secrets, private URLs, comments, and raw launchers, use the intentionally unencrypted [`omo-model-clone`](./EXACT_CLONE_MANUAL_INSTALL_GUIDE.md). For ordinary migration or sharing without credentials/endpoints, use the redacted [`omo-model-pack`](./CONFIG_PACK_RESTORE_GUIDE.md#create-a-pack). Never substitute one mode for the other: the exact clone carries raw secrets and requires explicit acknowledgements; the redacted pack is a sanitized template with no include-secrets option.
3. **Inspect, extract, and manually place a received archive:** Identify its format first. For `omo-model-clone`, follow the guarded [exact clone inspect/extract/manual-placement procedure](./EXACT_CLONE_MANUAL_INSTALL_GUIDE.md#recipient-inspect-and-extract-for-review) with a trusted Windows recipient. It provides no automatic installation, process management, or restart behavior. For `omo-model-pack`, follow [Inspect before extracting](./CONFIG_PACK_RESTORE_GUIDE.md#inspect-before-extracting) and its approval-gated manual merge. Keep the command lane and archive type consistent; inspection/extraction never authorizes displaying secrets.
4. **Upgrade any older GitHub version:** Follow [`UPGRADE.md`](./UPGRADE.md) from the beginning. It covers the original `1.0.0` Node checkout, GitHub-global installs, linked checkouts with customized profiles, and standalone Windows launchers. Inventory and back up the old installation before changing it; never overwrite active OpenCode provider configuration or discard customized profiles.
