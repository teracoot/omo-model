...
2. **Export configuration—owner chooses one mode:** For a trusted friend who must receive an exact working Windows environment, including secrets, private URLs, comments, and raw launchers, use the intentionally unencrypted [`omo-model-clone`](./EXACT_CLONE_RESTORE_GUIDE.md). For ordinary migration or sharing without credentials/endpoints, use the redacted [`omo-model-pack`](./CONFIG_PACK_RESTORE_GUIDE.md#create-a-pack). Never substitute one mode for the other: exact clone requires the owner’s explicit secret and unencrypted acknowledgements; redacted migration remains a sanitized template and has no include-secrets option.
3. **Inspect, extract, and restore a received archive:** Identify its format first. For `omo-model-clone`, follow the guarded [exact clone inspect/extract/restore procedure](./EXACT_CLONE_RESTORE_GUIDE.md#recipient-inspect-extract-and-restore) and use a trusted Windows recipient. For `omo-model-pack`, follow [Inspect before extracting](./CONFIG_PACK_RESTORE_GUIDE.md#inspect-before-extracting) and its approval-gated manual merge. Keep the command lane and archive type consistent; inspection/extraction never authorizes displaying secrets.
